...
board = cv2.aruco.CharucoBoard(charuco_length, charuco_width, 1, 0.8, aruco_dict)
...
def detect_charuco_board(image):
    ...
    aruco_square_corners, aruco_square_ids, rejectedImgPoints = cv2.aruco.detectMarkers(
        gray, aruco_dict
    )
    ...
    if len(aruco_square_corners) > 0:
        ...
        for this_corner in aruco_square_corners:
            cv2.cornerSubPix(
                gray, this_corner, winSize=(3, 3), zeroZone=(-1, -1), criteria=criteria
            )
        res2 = cv2.aruco.interpolateCornersCharuco(
            aruco_square_corners, aruco_square_ids, gray, board
        )
...
if __name__ == "__main__":
    ...
    try:
        multi_cam = FMC_MultiCamera(
            ...
            show_multi_cam_stream_bool=False,
        ...
        each_cameras_shared_board_view_count_array = np.zeros(
            (multi_cam.num_cams, multi_cam.num_cams)
        )
        ...
        while not multi_cam.exit_event.is_set():
            if not multi_cam.multi_cam_tuple_queue.empty():
                ...
                if multi_cam._save_each_cam_to_mp4:
                    ...
                    for this_cam_num in range(multi_cam.num_cams):
                        multi_cam._each_cam_video_writer_object_list[
                            this_cam_num
                        ].write(this_multi_cam_tuple[this_cam_num][1])

                new_cam_image_list = []
                full_charuco_detected_on_this_frame = [
                    False for _ in range(multi_cam.num_cams)
                ]
                ...
                for this_cam_num in range(multi_cam.num_cams):
                    ...
                    if current_loop_count % 10 == 0:
                        (
                            charuco_corners[this_cam_num],
                            charuco_ids[this_cam_num],
                            aruco_square_corners[this_cam_num],
                            aruco_square_ids[this_cam_num],
                        ) = detect_charuco_board(this_cam_image)
                    ...
                    current_cam_corner_count_str = (
                        this_cam_name
                        + ": "
                        + str(len(charuco_ids[this_cam_num]))
                        + " of "
                        + str(num_charuco_corners)
                        + " detected | Full Board: "
                        + str(full_charuco_detected_on_this_frame[this_cam_num])
                    )
                    this_cam_shared_views_str = " | Shared Views: " + str(
                        each_cameras_shared_board_view_count_total[this_cam_num]
                    )
                    text_to_write_on_this_camera = (
                        current_cam_corner_count_str + this_cam_shared_views_str
                    )

                    if this_cam_num == 0:
                        queue_size_str = "Multi-Camera Queue Size: " + str(
                            multi_cam.multi_cam_tuple_queue.qsize()
                        )
                    ...
                    if full_charuco_detected_on_this_frame[this_cam_num]:
                        cv2.polylines(
                            image_w_markers,
                            np.int32([charuco_corners[this_cam_num]]),
                            True,
                            (0, 100, 255),
                            2,
                        )
                    ...
                    charuco_points_from_previous_frames[this_cam_num].append(
                        charuco_corners[this_cam_num]
                    )
                        ...
                        for this_other_camera_num in range(multi_cam.num_cams):
                            if this_other_camera_num != this_cam_num:
                                if full_charuco_detected_on_this_frame[
                                    this_other_camera_num
                                ]:
                                    each_cameras_shared_board_view_count_array[
                                        this_cam_num, this_other_camera_num
                                    ] += 1
                ...
                for this_pair_num, this_cam_pair in enumerate(camera_pair_list):
                    ...
                    if (
                        full_charuco_detected_on_this_frame[camera_one_id]
                        and full_charuco_detected_on_this_frame[camera_two_id]
                    ):
                        camera_pair_joint_view_count[this_pair_num] += 1

                # update plots
                multi_cam_queue_size_list.append(
                    multi_cam.multi_cam_tuple_queue.qsize()
                )
                ...
                if (
                    cv2.getWindowProperty(multi_cam._rec_name, cv2.WND_PROP_VISIBLE) < 1
